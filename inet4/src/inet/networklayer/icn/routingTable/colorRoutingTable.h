...
class ColorRoutingTable : public cSimpleModule, protected cListener, public ILifecycle
{
    private:
        ...
        //路由表, 每个SID可能对应多条路径
        typedef std::multimap<SID_t, std::shared_ptr<Croute>>  Table;
        typedef std::map<shared_ptr<Croute>,SID_t> ReverseTable;
        //为每个路由表项维护一个生存周期值
    ...
    public:
        ...
        ~ColorRoutingTable();
        //创建一条路由表向
        shared_ptr<Croute> CreateEntry(SID_t sid, NID_t nid, simtime_t t );
        ...
        //通过SID查找路由表
        shared_ptr<Croute> findMachEntry(SID_t sid);

        //通过route指针删除路由表项
        void removeEntry(shared_ptr<Croute> croute);

        //通过SID删除路由表项
        void removeEntry(SID_t sid);
    protected:
