...
void Aodv_cluster::handleMessageWhenUp(cMessage *msg)
{
...
INetfilter::IHook::Result Aodv_cluster::ensureRouteForDatagram(Packet *datagram)
{
    ...
    if (destAddr.isBroadcast() || routingTable->isLocalAddress(destAddr) || destAddr.isMulticast())
        ...
        AodvRouteData_cluster *routeData = route ? dynamic_cast<AodvRouteData_cluster *>(route->getProtocolData()) : nullptr;
...
bool Aodv_cluster::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void Aodv_cluster::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address Aodv_cluster::getSelfIPAddress() const
{
...
void Aodv_cluster::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> Aodv_cluster::createRREQ(const L3Address& destAddr)
{
    ...
    if (lastKnownRoute && lastKnownRoute->getSource() == this) {
        ...
        AodvRouteData_cluster *routeData = check_and_cast<AodvRouteData_cluster *>(lastKnownRoute->getProtocolData());
...
const Ptr<Rrep> Aodv_cluster::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void Aodv_cluster::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    AodvRouteData_cluster *destRouteData = nullptr;
    ...
    if (destRoute && destRoute->getSource() == this) {    // already exists
        destRouteData = check_and_cast<AodvRouteData_cluster *>(destRoute->getProtocolData());
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                if (nextHopToDestRoute && nextHopToDestRoute->getSource() == this) {
                    AodvRouteData_cluster *nextHopToDestRouteData = check_and_cast<AodvRouteData_cluster *>(nextHopToDestRoute->getProtocolData());
...
void Aodv_cluster::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
    ...
    AodvRouteData_cluster *routingData = check_and_cast<AodvRouteData_cluster *>(route->getProtocolData());
...
void Aodv_cluster::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    if (!reverseRoute || reverseRoute->getSource() != this) {    // create
        ...
        AodvRouteData_cluster *routeData = check_and_cast<AodvRouteData_cluster *>(reverseRoute->getProtocolData());
...
void Aodv_cluster::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void Aodv_cluster::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
    ...
    AodvRouteData_cluster *unreachableRouteData = check_and_cast<AodvRouteData_cluster *>(unreachableRoute->getProtocolData());
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        AodvRouteData_cluster *routeData = dynamic_cast<AodvRouteData_cluster *>(route->getProtocolData());
...
void Aodv_cluster::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void Aodv_cluster::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void Aodv_cluster::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void Aodv_cluster::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void Aodv_cluster::completeRouteDiscovery(const L3Address& target)
{
...
void Aodv_cluster::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> Aodv_cluster::createHelloMessage()
{
...
void Aodv_cluster::sendHelloMessagesIfNeeded()
{
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        ...
        if (route->getSource() == this) {
            AodvRouteData_cluster *routeData = check_and_cast<AodvRouteData_cluster *>(route->getProtocolData());
...
void Aodv_cluster::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
    ...
    if (!routeHelloOriginator || routeHelloOriginator->getSource() != this)
        ...
        AodvRouteData_cluster *routeData = check_and_cast<AodvRouteData_cluster *>(routeHelloOriginator->getProtocolData());
...
void Aodv_cluster::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void Aodv_cluster::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result Aodv_cluster::datagramForwardHook(Packet *datagram)
{
    ...
    AodvRouteData_cluster *routeDestData = routeDest ? dynamic_cast<AodvRouteData_cluster *>(routeDest->getProtocolData()) : nullptr;
...
void Aodv_cluster::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
    ...
    IRoute *unreachableRoute = routingTable->findBestMatchingRoute(unreachableAddr);
    AodvRouteData_cluster *unreachableRouteData = unreachableRoute ? dynamic_cast<AodvRouteData_cluster *>(unreachableRoute->getProtocolData()) : nullptr;
...
void Aodv_cluster::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool Aodv_cluster::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData_cluster *routeData = check_and_cast<AodvRouteData_cluster *>(route->getProtocolData());
...
void Aodv_cluster::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void Aodv_cluster::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void Aodv_cluster::handleBlackListTimer()
{
